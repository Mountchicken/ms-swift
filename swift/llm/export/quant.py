...
class QuantEngine(ProcessorMixin):
    ...
    def quantize(self):
        ...
        if args.quant_bits is None and args.quant_method != "fp8":
            raise ValueError(
                f"Please set the quant_bits. args.quant_bits: {args.quant_bits}"
            )
        if args.quant_method == "awq":
            ...
            self.model.save_quantized(
                args.output_dir,
                safetensors=args.safe_serialization,
    ...
    def awq_model_quantize(self) -> None:
        ...
        if self.model.model_info.is_moe_model:
            quant_config["modules_to_not_convert"] = (
                self.args.get_modules_to_not_convert()
            )
        ...
        with self._patch_awq_move_embed(self.model):
            self.model.quantize(
                ...
                n_parallel_calib_samples=args.quant_batch_size,
            )
    ...
    def get_block_name_to_quantize(model: nn.Module) -> Optional[str]:
        ...
        if hasattr(model_arch, "language_model"):
            assert (
                len(model_arch.language_model) == 1
            ), f"mllm_arch.language_model: {model_arch.language_model}"
        ...
        for n, m in model.named_modules():
            if (
                isinstance(m, (nn.ModuleList, nn.Sequential))
                and len(m) >= 10
                and "mlp" not in m[0].__class__.__name__.lower()
            ):  # fix moe
    ...
    def gptq_model_quantize(self, v2: bool = False):
        ...
        modules_in_block_to_quantize = self.get_modules_in_block_to_quantize(
            self.model, block_name_to_quantize
        )
