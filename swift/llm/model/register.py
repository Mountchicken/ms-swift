...
from .patcher import (
    ...
    patch_automodel,
    patch_automodel_for_sequence_classification,
...
GetModelTokenizerFunction = Callable[
            ...
            for model in model_group.models:
                for key in ["ms_model_id", "hf_model_id", "model_path"]:
                    ...
                    if (
                        ...
                        and model_name == value.rsplit("/", 1)[-1].lower()
                    ):
                        return model_group
...
def register_model(model_meta: ModelMeta, *, exist_ok: bool = False) -> None:
    ...
    from .constant import MLLMModelType, RerankerModelType, RMModelType
...
def load_by_unsloth(args):
    ...
    assert (
        is_unsloth_available()
    ), "please install unsloth if using `use_unsloth=True`: `pip install unsloth`"
...
def _patch_awq_compat(model_info):
    if (
        version.parse(transformers.__version__) < version.parse("4.50")
        or model_info.quant_method != "awq"
    ):
        return
    ...
    try:
        ...
        AwqQuantizer._process_model_before_weight_loading = (
            _new_process_model_before_weight_loading
        )
    except Exception:
        pass
...
def get_model_tokenizer_from_local(
    ...
    **kwargs,
    ...
    HfConfigFactory.set_config_attr(
        model_config, "torch_dtype", torch_dtype, include_vit=True
    )
    ...
    if load_model:
        ...
        if (
            model_info.task_type in {"seq_cls", "reranker"}
            and automodel_class is None
            and not return_dummy_model
        ):
            with patch_automodel_for_sequence_classification(
                model_config=model_config, patch_from_pretrained=False
            ):
                try:
        ...
        if model is None:
            if return_dummy_model:
                ...
                context = partial(
                    patch_automodel_for_sequence_classification, **context_kwargs
                )
            elif (
                model_info.task_type == "seq_cls"
                and model_meta.is_reward
                and model_config.num_labels > 1
            ):
                logger.warning(
                    "You are using a reward model for seq_cls task and num_labels > 1, "
                ...
                logger.info(
                    "Converting CausalLM to SequenceClassification for reranker task with num_labels=1"
                )
                context = partial(
                    patch_automodel_for_sequence_classification, **context_kwargs
                )
        ...
        has_remote_code = (
            ...
            and automodel_class.__name__ in model_config.auto_map
        )
        if has_remote_code and model._auto_class is None:
            model._auto_class = automodel_class.__name__

        if (
            model_info.task_type == "embedding"
            and automodel_class.__name__ != "AutoModel"
        ):
...
def get_model_tokenizer_sentence_transformers(
    model_dir: str,
    model_info: ModelInfo,
    model_kwargs: Dict[str, Any],
    load_model: bool = True,
    ...
    **kwargs,
    ...
    if load_model:
        ...
        def enable_input_require_grads(self):
            ...
            self._require_grads_hook = self[
                0
            ].auto_model.embed_tokens.register_forward_hook(make_inputs_require_grads)
...
def get_model_tokenizer_with_flash_attn(
    ...
    **kwargs,
    ...
    if model_config is None:
        model_config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True)
    AttnImpl.update_attn_impl(
        model_config, kwargs.get("attn_impl"), kwargs.get("attn_impl_keys")
    )
...
def get_default_device_map():
    if (
        ...
        or os.environ.get("ACCELERATE_USE_FSDP", "False") == "true"
    ):
        return None
...
def _get_model_info(
    model_dir: str, model_type: Optional[str], quantization_config
) -> ModelInfo:
    try:
    ...
    if model_type is None:
        ...
        if len(model_types) > 1:
            raise ValueError(
                "Please explicitly pass the model_type. For reference, "
        ...
        raise ValueError(
            f"model_type: '{model_type}' not in {list(MODEL_MAPPING.keys())}"
        )
...
def get_model_info_meta(
    ...
    **kwargs,
    ...
    model_info = _get_model_info(
        model_dir, model_type, quantization_config=quantization_config
    )
    ...
    if model_meta is None:
        model_meta = ModelMeta(
            None, [], "dummy", get_model_tokenizer_from_local, model_arch=None
        )
        logger.info(f"Temporarily create model_meta: {model_meta}")

    if torch_dtype is None:
        torch_dtype = model_meta.torch_dtype or get_default_torch_dtype(
            model_info.torch_dtype
        )
...
def get_model_tokenizer(
    ...
    *,
    ...
    automodel_class=None,
    task_type: Literal[
        "causal_lm", "seq_cls", "reranker", "generative_reranker"
    ] = None,
    ...
    **kwargs,
    ...
    patch_offload_context = (
        patch_attach_align_device_hook_on_blocks() if patch_offload else nullcontext()
    )
    ...
    if not isinstance(processor, PreTrainedTokenizerBase) and hasattr(
        processor, "tokenizer"
    ):
    ...
    if new_special_tokens:
        ...
        if num_new_tokens > 0:
            ...
            if model is not None and not return_dummy_model:
                ...
                origin_vocab_size = HfConfigFactory.get_config_attr(
                    llm_model.config, "vocab_size"
                )
                if origin_vocab_size < len(tokenizer):
                    ...
                    HfConfigFactory.set_config_attr(
                        llm_model.config, "vocab_size", vocab_size
                    )
    ...
    if model is not None:
        ...
        if not hasattr(model, "generation_config") and os.path.isfile(
    ...
    processor.tokenizer.use_fast = False
    return model, processor
